...
void build_one_sample(int idx) {
	...
	/*for (int i = 0; i < sample_size; i++) {
		int idx = gene_idx();
		while (all_chosen.find(idx) != all_chosen.end())
			idx = (idx + 7 + rand() % 2) % C;
		all_chosen.insert(idx);
	}*/
...
int main(int argc, char* argv[]) {
	...
	if(argc < no_arg)
	{
		fprintf(stderr,"%s Full_path_of_RAxML inputFile1\n\n", argv[0]);
		...
		fprintf(stderr,"  -t N specify number of threads to be allocated for raxml calls >= 2 (10)\n");
		fprintf(stderr,"  -p C Full_Path_of_program: choices are I for IQtree or F for FastTree followed by full path of the executible (R for RAxML)\n  	A \"!\" for Full_Path_of_program indicates the current directory\n\n");
		exit(1);
	}
